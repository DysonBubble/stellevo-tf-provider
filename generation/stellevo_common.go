...
func DoMain(generationKind string, provider string, resources map[string]*schema.Resource, dataSources map[string]*schema.Resource) {
	switch generationKind {
		case "inputs":
			PrintInputTS(provider, GetResourceDictionary(resources, dataSources), GetResourceName(TS_ALL_RESOURCES_TYPE_NAME, TS_ALL_DATA_SOURCES_TYPE_NAME), TS_SCHEMA_INPUT_SENSITIVE_STRING)
			break
		case "outputs":
			PrintOutputTS(GetResourceDictionary(resources, dataSources), GetResourceName(TS_ALL_RESOURCES_TYPE_NAME, TS_ALL_DATA_SOURCES_TYPE_NAME))
			break
		case "resources":
			fmt.Println(GetTSResourcesLookupValue(resources, dataSources))
			break
		case "codegen":
...
func PrintInputTS(provider string, resources map[string]*schema.Resource, typeName string, sensitiveStringSchemaName string) {
	fmt.Println(GetTSInputPrelude(provider, resources, typeName ))
...
func PrintOutputTS(resources map[string]*schema.Resource, typeName string) {
	fmt.Println(GetTSOutputPrelude( resources, typeName ))
	sortedKeys := SortedStringKeys(resources)
  for _, res_id := range sortedKeys {
...
func GetTSInputPrelude(provider string, resources map[string]*schema.Resource, typeName string) string {
	return fmt.Sprintf("// THIS FILE IS AUTOGENERATED BY GO SCRIPT\n\nimport * as " + TS_PLATFORM_API_IMPORT_NAME + " from '../../../common/platforms/resources';\nimport * as " + TS_PROVIDER_RESOURCES_IMPORT_NAME + " from '../resources/';\nimport * as " + TS_SCHEMA_COMMON_IMPORT_NAME + " from '../../../common/providers/schemas';\n%s\n%s\n%s %s", ProviderSpecificInputPrelude(), GetTSInputOrOutputLookupType(resources, typeName), "export const " + TS_SCHEMA_INPUT_SENSITIVE_STRING + " = ", ProviderSpecificSensitiveStringSchemaDefinition(TS_SCHEMA_INPUT_SENSITIVE_STRING_NAME))
}
...
func GetTSOutputPrelude(resources map[string]*schema.Resource, typeName string) string {
	return fmt.Sprintf("// THIS FILE IS AUTOGENERATED BY GO SCRIPT\n\nimport * as " + TS_PLATFORM_API_IMPORT_NAME + " from '../../../common/platforms/resources';\nimport * as " + TS_SCHEMA_COMMON_IMPORT_NAME + " from '../../../common/providers/schemas';\n%s", GetTSInputOrOutputLookupType(resources, typeName))
}
...
func GetTSResourcesLookupValue(resources map[string]*schema.Resource, dataSources map[string]*schema.Resource) string {
	ret := fmt.Sprintf("// THIS FILE IS AUTOGENERATED BY GO SCRIPT\n\nexport const %s = {\n", TS_ALL_RESOURCES_VAR_NAME)
	sortedKeys := SortedStringKeys(resources)
	for _, res_id := range sortedKeys {
		ret = fmt.Sprintf("%s  %q: %q,\n", ret, res_id, res_id)
	}
	ret = fmt.Sprintf("%s} as const;\nexport const %s = {\n", ret, TS_ALL_DATA_SOURCES_VAR_NAME)
	sortedKeys = SortedStringKeys(dataSources)
	for _, data_id := range sortedKeys {
		ret = fmt.Sprintf("%s  %q: %q,\n", ret, data_id, data_id)
	}
...
const TS_ALL_DATA_SOURCES_TYPE_NAME = "TAllDataSources"
const TS_ALL_RESOURCES_VAR_NAME = "AllResources"
const TS_ALL_DATA_SOURCES_VAR_NAME = "AllDataSources"
